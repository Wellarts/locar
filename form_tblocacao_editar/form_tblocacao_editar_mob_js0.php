<form name="F2" method=post 
               ...
               target="_self"> 
<input type="hidden" name="idlocacao" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idlocacao']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tblocacao_editar_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tblocacao_editar_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var Crtl_btn_finalizaLocacao = false;
function sc_btn_finalizaLocacao()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_finalizaLocacao(); }, 500);
      return;
    }
    if (Crtl_btn_finalizaLocacao) {return;}
    if (confirm("<?php echo html_entity_decode('Desejar finalizar esta locação?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>"))
    {
        Crtl_btn_finalizaLocacao = true;
        document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?finalizaLocacao?@?";
        document.F1.action = "form_tblocacao_editar_mob.php";
...
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_fichaLocacao(); }, 500);
      return;
    }
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?fichaLocacao?@?";
    document.F1.action = "form_tblocacao_editar_mob.php";
    document.F1.target = "_blank";
    document.F1.submit();
}
var Crtl_btn_locacaoFinalizada = false;
function sc_btn_locacaoFinalizada()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_locacaoFinalizada(); }, 500);
      return;
    }
    if (Crtl_btn_locacaoFinalizada) {return;}
    if (confirm("<?php echo html_entity_decode('Deseja realmente Reabrir a Locação?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>"))
    {
        Crtl_btn_locacaoFinalizada = true;
        document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?locacaoFinalizada?@?";
        document.F1.action = "form_tblocacao_editar_mob.php";
        document.F1.target = "_self";
        document.F1.submit();
    }
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tblocacao_editar']['where_detal']))
  {
?>  
