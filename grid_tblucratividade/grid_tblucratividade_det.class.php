<?php
//--- 
class grid_tblucratividade_det
{
   var $Ini;
   var $Erro;
   var $Db;
 ...
 function monta_det()
 {
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tblucratividade']['seq_dir'] = 0; 
    $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tblucratividade']['sub_dir'] = array(); 
   ...
   $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_edit.php", "F", "nmgp_Form_Num_Val") ; 
       $nmgp_select = "SELECT idLucratividade, idFKLocacao, valorDiaria, dataDiaria, idFKveiculo from " . $this->Ini->nm_tabela; 
   $parms_det = explode("*PDet*", $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tblucratividade']['chave_det']) ; 
   ...
   $nmgp_select .= " where  idFKLocacao = $parms_det[0] and idLucratividade = $parms_det[1]" ;  
   ...
   $this->idlucratividade = $rs->fields[0] ;  
   $this->idlucratividade = (string)$this->idlucratividade;
   $this->idfklocacao = $rs->fields[1] ;  
   $this->idfklocacao = (string)$this->idfklocacao;
   $this->valordiaria = $rs->fields[2] ;  
   $this->valordiaria = str_replace(",", ".", $this->valordiaria);
   $this->valordiaria = (string)$this->valordiaria;
   $this->datadiaria = $rs->fields[3] ;  
   $this->idfkveiculo = $rs->fields[4] ;  
   $this->idfkveiculo = (string)$this->idfkveiculo;
   if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tblucratividade']['cmp_acum']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tblucratividade']['cmp_acum']))
   {
       $parms_acum = explode(";", $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tblucratividade']['cmp_acum']);
...
//--- 
   ...
   if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tblucratividade']['det_print'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
   {
   ...
   else
   {
       ...
       $nm_saida->saida(" <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "grid_tblucratividade/grid_tblucratividade_det_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
   if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_tblucratividade']['pdf_det'] && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tblucratividade']['det_print'] != "print")
   {
   ...
   $nm_saida->saida("    	<tr>\r\n");
   $nm_saida->saida("        	<td id=\"lin1_col1\" class=\"scGridHeaderFont\"><span>Valores - Locações</span></td>\r\n");
   ...
   $nm_saida->saida("  </TD>\r\n");
   $nm_saida->saida(" </TR>\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tblucratividade']['det_print'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_tblucratividade']['pdf_det']) 
   { 
       $nm_saida->saida("   <tr><td class=\"scGridTabelaTd\">\r\n");
       ...
       {
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Dpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_tblucratividade/grid_tblucratividade_config_pdf.php?nm_opc=pdf_det&nm_target=0&nm_cor=cor&papel=1&orientacao=1&largura=1200&conf_larg=S&conf_fonte=10&language=pt_br&conf_socor=S&KeepThis=false&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
       }
       if ($this->nmgp_botoes['det_print'] == "on")
       {
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bprint", "", "", "Dprint_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_tblucratividade/grid_tblucratividade_config_print.php?nm_opc=detalhe&nm_cor=AM&language=pt_br&KeepThis=true&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
       }
   ...
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
          ...
          $conteudo = trim(NM_encode_input(sc_strip_script($this->idlucratividade))); 
          if ($conteudo === "") 
          { 
              $conteudo = "&nbsp;" ; 
          } 
   ...
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_idlucratividade_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldOddVert css_idlucratividade_det_line\"  NOWRAP ALIGN=\"\" VALIGN=\"\">" . $conteudo . "</TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   ...
   $nm_saida->saida("    <TD class=\"scGridFieldOddVert css_valordiaria_det_line\"  NOWRAP ALIGN=\"\" VALIGN=\"\">" . $conteudo . "</TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
          $SC_Label = (isset($this->New_label['datadiaria'])) ? $this->New_label['datadiaria'] : "Data da Saída"; 
          $conteudo = trim(NM_encode_input(sc_strip_script($this->datadiaria))); 
          if ($conteudo === "") 
          { 
              $conteudo = "&nbsp;" ; 
          } 
   ...
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_datadiaria_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldEvenVert css_datadiaria_det_line\"  NOWRAP ALIGN=\"\" VALIGN=\"\">" . $conteudo . "</TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
          $SC_Label = (isset($this->New_label['idfkveiculo'])) ? $this->New_label['idfkveiculo'] : "Veículo"; 
          $conteudo = trim(NM_encode_input(sc_strip_script($this->idfkveiculo))); 
          if ($conteudo === "") 
          { 
              $conteudo = "&nbsp;" ; 
          } 
          $this->Lookup->lookup_idfkveiculo($conteudo , $this->idfkveiculo) ; 
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_idfkveiculo_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldOddVert css_idfkveiculo_det_line\"  NOWRAP ALIGN=\"\" VALIGN=\"\">" . $conteudo . "</TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("</TABLE>\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tblucratividade']['det_print'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_tblucratividade']['pdf_det']) 
...
//--- 
   ...
   $nm_saida->saida("   {\r\n");
   $nm_saida->saida("       NovaJanela = window.open (\"grid_tblucratividade_doc.php?script_case_init=" . NM_encode_input($this->Ini->sc_page) . "&script_case_session=" . session_id() . "&nm_cod_doc=\" + campo1 + \"&nm_nome_doc=\" + campo2 + \"&nm_cod_apl=\" + campo3, \"ScriptCase\", \"resizable\");\r\n");
   $nm_saida->saida("   }\r\n");
   $nm_saida->saida("   function nm_gp_move(x, y, z, p, g) \r\n");
   $nm_saida->saida("   {\r\n");
   $nm_saida->saida("      window.location = \"" . $this->Ini->path_link . "grid_tblucratividade/index.php?nmgp_opcao=pdf_det&nmgp_tipo_pdf=\" + z + \"&nmgp_parms_pdf=\" + p +  \"&nmgp_graf_pdf=\" + g + \"&script_case_init=" . NM_encode_input($this->Ini->sc_page) . "&script_case_session=" . session_id() . "\";\r\n");
   $nm_saida->saida("   }\r\n");
   $nm_saida->saida("   function nm_gp_print_conf(tp, cor)\r\n");
   $nm_saida->saida("   {\r\n");
   $nm_saida->saida("       window.open('" . $this->Ini->path_link . "grid_tblucratividade/grid_tblucratividade_iframe_prt.php?path_botoes=" . $this->Ini->path_botoes . "&script_case_init=" . NM_encode_input($this->Ini->sc_page) . "&script_case_session=" . session_id() . "&opcao=det_print&cor_print=' + cor,'','location=no,menubar,resizable,scrollbars,status=no,toolbar');\r\n");
   $nm_saida->saida("   }\r\n");
