...
class grid_tbveiculos_xml
{
   ...
   function inicializa_vars()
   {
      ...
      $this->Arquivo     .= ".xml";
      $this->Tit_doc      = "grid_tbveiculos.xml";
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbveiculos']['xml_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbveiculos']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbveiculos']['xml_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbveiculos']['xml_name']);
      }
      if (!$this->Grava_view)
      {
      ...
      while (!$rs->EOF)
      {
         $this->xml_registro = "<grid_tbveiculos";
   ...
   function NM_export_idveiculo()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->idveiculo))
         {
             $this->idveiculo = sc_convert_encoding($this->idveiculo, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " idveiculo =\"" . $this->trata_dados($this->idveiculo) . "\"";
   }
   //----- idfkmarcaveiculo
   function NM_export_idfkmarcaveiculo()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->look_idfkmarcaveiculo))
         {
             $this->look_idfkmarcaveiculo = sc_convert_encoding($this->look_idfkmarcaveiculo, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " idfkmarcaveiculo =\"" . $this->trata_dados($this->look_idfkmarcaveiculo) . "\"";
   }
   //----- modeloveiculo
   function NM_export_modeloveiculo()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->modeloveiculo))
         {
             $this->modeloveiculo = sc_convert_encoding($this->modeloveiculo, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " modeloveiculo =\"" . $this->trata_dados($this->modeloveiculo) . "\"";
   }
   //----- anoveiculo
   function NM_export_anoveiculo()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->anoveiculo))
         {
             $this->anoveiculo = sc_convert_encoding($this->anoveiculo, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " anoveiculo =\"" . $this->trata_dados($this->anoveiculo) . "\"";
   }
   //----- placaveiculo
   function NM_export_placaveiculo()
   {
         $this->nm_gera_mask($this->placaveiculo, "xxx-xxxx"); 
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->placaveiculo))
         {
             $this->placaveiculo = sc_convert_encoding($this->placaveiculo, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " placaveiculo =\"" . $this->trata_dados($this->placaveiculo) . "\"";
   }
   //----- corveiculo
   function NM_export_corveiculo()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->corveiculo))
         {
             $this->corveiculo = sc_convert_encoding($this->corveiculo, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " corveiculo =\"" . $this->trata_dados($this->corveiculo) . "\"";
   }
   //----- kmatualveiculo
   function NM_export_kmatualveiculo()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->kmatualveiculo))
         {
             $this->kmatualveiculo = sc_convert_encoding($this->kmatualveiculo, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " kmatualveiculo =\"" . $this->trata_dados($this->kmatualveiculo) . "\"";
   }
   //----- statusveiculo
   function NM_export_statusveiculo()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->look_statusveiculo))
         {
             $this->look_statusveiculo = sc_convert_encoding($this->look_statusveiculo, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " statusveiculo =\"" . $this->trata_dados($this->look_statusveiculo) . "\"";
   }
   ...
   function monta_html()
   {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbveiculos']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbveiculos']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
...
<HEAD>
 <TITLE>Consulta de Veículos :: XML</TITLE>
